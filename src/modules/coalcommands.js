...
defineChatCommand(
  ...
  async (interaction, options) => {
    ...
    await interaction.reply(
      `<@${options.user.id}> GET MINING!!!! ${options.coal} COAL TO GO!!!!!!`
    );
  }
);
...
defineChatCommand(
  ...
  async (interaction, options) => {
    ...
    return await interaction.reply({
      content: `No existing punishment`,
      ephemeral: true,
    });
  }
);
...
defineChatCommand(
  ...
  async (interaction, options) => {
    ...
    if (record) {
      ...
      return await interaction.reply(
        `You ${won ? "won" : "lost"}! Your coal amount is now ${newCoal}`
      );
    }
    return await interaction.reply({
      content: `No existing punishment`,
...
defineChatCommand(
  ...
  async (interaction, options) => {
    ...
    if (useroccurs) {
      ...
      return await interaction.reply(
        `You freed <@${options.user.id}>! Lucky them.`
      );
    }
    return await interaction.reply({
      content: `That user isn't mining.`,
      ephemeral: true,
    });
  }
);
...
defineChatCommand(
  ...
  async (interaction, options) => {
    let record = database.data.find((x) => x.id == options.user.id);
    let isYou = options.user.id == interaction.user.id;
    let alreadyHelping = database.data.find(
      (x) => x.helpid && x.id == interaction.user.id
    );

    if (record && !isYou && !alreadyHelping) {
      database.data = [
        ...database.data,
        {
          id: interaction.user.id,
      ...
      return await interaction.reply(
        `<@${interaction.user.id}> is helping <@${options.user.id}> mine coal! How nice.`
      );
    }
    return await interaction.reply({
      content: alreadyHelping
        ? `You're already helping someone!`
        : isYou
        ? `You can't help yourself!`
        : `You can't help that user`,
      ephemeral: true,
    });
  }
);
...
defineChatCommand(
  ...
  async (interaction, options) => {
    ...
    if (record && record.helpid) {
      ...
      return await interaction.reply(
        `<@${interaction.user.id}> is no longer helping <@${record.helpid}>. Sadge.`
      );
    }
    return await interaction.reply({
      content: `You're not helping anyone!`,
      ephemeral: true,
    });
  }
);
